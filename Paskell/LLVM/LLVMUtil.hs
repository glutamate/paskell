...
withModule :: Module -> (Module -> IO a) -> IO a
withModule modul f = f ( modul)

createModule :: String -> IO Module
createModule name =
    withCString name $ \ namePtr -> do
     FFI.moduleCreateWithName namePtr
...
destroyModule = FFI.disposeModule 
...
ret bld e1 = withBuilder bld $ \bldref -> do
               FFI.buildRet bldref e1

iadd bld e1 e2 = withBuilder bld $ \bldref -> do
